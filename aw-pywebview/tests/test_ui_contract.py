...
def test_frontend_daily_summary_hooks_exist():
    ...
    assert "renderXAxis();" in app_js
    assert "renderYAxis(" in app_js
...
def test_frontend_daily_summary_layout_containers_exist():
    ...
    assert 'class="workspace full-width-stack"' in index_html
    assert 'class="stats-strip full-span-stats"' in index_html
    assert 'class="stat-chip card-shell translucent"' in index_html
    assert 'id="activeDuration"' in index_html
    assert 'id="appCount"' in index_html
    assert 'id="avgSwitchRate"' in index_html
    assert 'class="chart-panel card-shell full-width-card"' in index_html
    assert 'class="chart-board full-width-chart-board"' in index_html
    assert 'class="distribution-panel card-shell full-width-card"' in index_html
    assert 'class="distribution-layout"' in index_html
    assert 'class="donut-wrap large-donut-wrap"' in index_html
    ...
    assert 'id="hourlyChart" class="hourly-chart full-width-hourly-chart"' in index_html
    assert 'id="hourlyYAxis"' in index_html
    assert 'id="hourlyXAxis"' in index_html
    assert 'id="hourlyTooltip"' in index_html
    ...
    assert 'class="dashboard-layout"' not in index_html
